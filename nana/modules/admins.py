    ...
    if message.chat.type in ["group", "supergroup"]:
        ...
        if can_pin:
            try:
                ...
                await edit_or_reply(message, text="`invalid username or userid`")
                return
    ...
    if message.chat.type in ["group", "supergroup"]:
        ...
        if can_pin:
            try:
                ...
                await edit_or_reply(message, text="`Error!`\n" f"**Log:** `{e}`")
                return
        else:
            await edit_or_reply(message, text=tld("denied_permission"))
            await asyncio.sleep(5)
...
@app.on_message(
    filters.user(AdminSettings) & filters.command("promote", COMMAND_PREFIXES)
)
...
@app.on_message(
    filters.user(AdminSettings) & filters.command("demote", COMMAND_PREFIXES)
)
    ...
    if message.chat.type in ["group", "supergroup"]:
        ...
        if lock_type == "all":
            try:
                ...
                await edit_or_reply(message, text=tld("lock_all"))
                await asyncio.sleep(5)
                await message.delete()
...
@app.on_message(
    filters.user(AdminSettings) & filters.command("unlock", COMMAND_PREFIXES)
)
...
@app.on_message(
    filters.user(AdminSettings) & filters.command("vlock", COMMAND_PREFIXES)
)
...
@app.on_message(
    filters.user(AdminSettings) & filters.command("delacc", COMMAND_PREFIXES)
)
